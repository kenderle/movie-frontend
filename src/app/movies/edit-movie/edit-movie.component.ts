...
export class EditMovieComponent implements OnInit, OnDestroy {
      ...
      this.subs.add(
        this.movieService.createMovie(params).subscribe(data => {
          if (data) {
            ...
            Swal.fire({
              icon: 'success',
              title: 'This movie has been successfully edited!',
          ...
          if (error) {
            ...
            this.submitting = false
            this.hasError = true
            this.errorMsg = "Something went wrong while trying to edit that movie!"
          }
        })
      )
    } else {
  ...
  editMovie() {
    const params = {
      ...
      releaseDate: this.formValues.release_date,
      rating: this.formValues.parental_rating,
      totalGross: this.formValues.total_gross,
      duration: this.formValues.duration,
      cast: this.formValues.cast,
      director: this.formValues.director
    }
    this.subs.add(
